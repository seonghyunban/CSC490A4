...
We conduct exploratory analysis on two baselines using temperature=1.0 sampling---the same setting used during RL training---to observe the model's full output distribution, not just its greedy best path. This reveals failure modes that binary correctness reward may have not been sufficient.
...
RL evaluation uses temperature=1.0 (required for REINFORCE), unlike Part~2 and Part~3's greedy evaluation, so accuracy numbers are not directly comparable across parts.
Greedy evaluation of this same checkpoint (Part~3) shows 82.94\% parseable---the model has acquired formatting skill during RL.
But under temperature=1.0 sampling, only 39.7\% of responses are parseable: the skill is fragile, concentrated along the highest-probability path.
During training, the 60.3\% unparseable samples receive zero correctness reward and produce no gradient, so the optimizer cannot reinforce what the model already knows how to do.
This motivates a format reward that provides signal to unparseable outputs, converting fragile formatting knowledge into a robust skill.
