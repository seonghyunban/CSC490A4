...
At the full-dataset level, the evaluation reports a strict exact rate of \textbf{17.89\%}, a relaxed numeric match rate of \textbf{18.04\%}, and a parseable rate of \textbf{82.94\%}. From these values, we derive four mutually exclusive categories. First, \textbf{strictly correct} outputs account for \textbf{17.89\%} of the dataset. Second, \textbf{relaxed-only correct} outputs account for only \textbf{0.15\%}, computed as the difference between relaxed and strict accuracy. Third, \textbf{extraction failures} account for \textbf{17.06\%}, corresponding to outputs for which the evaluator could not recover a valid strict final answer. Finally, the largest category is \textbf{parseable but incorrect} outputs, which account for \textbf{65.05\%} of the dataset.

This quantitative breakdown suggests that the main limitation of the Part 3 RL baseline is not widespread failure to produce any parseable answer. In fact, the model remains parseable on most problems. Instead, the dominant failure mode is that the model often produces an answer in the expected format, but the answer is still wrong. This means that the bottleneck is not purely output formatting; reasoning quality and final-step reliability remain the main challenges.

We also inspected the debug sample included in the evaluation output to obtain a more qualitative view of these incorrect but parseable responses. Within this subset, several recurring patterns appear. Some outputs follow a plausible reasoning path but make an arithmetic or bookkeeping error near the end. Others identify relevant quantities but fail to complete the derivation successfully. A smaller number appear truncated, malformed, or clearly off-task. These observations suggest that the broad ``parseable but incorrect'' category can be further decomposed into finer reasoning failures, but that decomposition currently relies on manual inspection rather than a fully automated classifier.

Overall, the error analysis supports two conclusions. First, correctness-only RL does produce meaningful learning signal: the model reaches nontrivial accuracy and maintains a high parseability rate. Second, most remaining failures come from incorrect reasoning despite producing a parseable answer, rather than from total formatting collapse. This helps motivate Part 4, where improved reward design should aim not only to increase correctness, but also to reduce these dominant reasoning-level failures and improve single-sample reliability.

\begin{table}[h]
\centering
\caption{Quantitative error breakdown for \texttt{a4p3-rl-baseline@466} on GSM8K.}
\label{tab:p3_error_breakdown}
\begin{tabular}{l r}
\hline
\textbf{Category} & \textbf{Proportion} \\
\hline
Strictly correct & 17.89\% \\
Relaxed-only correct & 0.15\% \\
Extraction failure & 17.06\% \\
Parseable but incorrect & 65.05\% \\
\hline
\end{tabular}
\end{table}

Table~\ref{tab:p3_error_breakdown} summarizes the evaluator-defined error categories derived directly from the saved GSM8K evaluation output.
